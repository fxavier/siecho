    ...
    def get_arv_dispensing(self, instance):
        ...
        try:
            response = requests.get(
                instance, params=params, auth=Constants.openmrs_auth.value)
        ...
        if data_list is not None:
            for data in data_list:
                visit, created = Visit.objects.get_or_create(
                    ...
                    appointment_date=datetime.fromtimestamp(
                        data['dispensing_date'] / 1e3),
                    next_appointment_date=datetime.fromtimestamp(
                        data['next_dispensing_date'] / 1e3),
                    gender=data['gender'],
        ...
        if data_list is not None:
            for data in data_list:
                missed_appointment, created = MissedAppointment.objects.get_or_create(
                    ...
                    last_appointment_date=DataConversion.convert_int_date(
                        data['ultimo_lev']),
                    gender=data['gender'],
                    community=data['Bairro'],
...
class PostData:
    ...
    def post_missed_appointment(cls):
        ...
        missed_appointment = MissedAppointment.objects.exclude(phone_number=None).filter(
            synced=False, drug_pickup_missed_days__gt=0, visit_missed_days__gt=0, last_appointment_date__gt='2023-01-01')
